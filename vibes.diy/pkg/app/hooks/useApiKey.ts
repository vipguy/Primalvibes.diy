...
export function useApiKey() {
  const [apiKey, setApiKey] = useState<{ key: string; hash: string } | null>(
    null,
  );
  ...
  const fetchNewKeyInternal = useCallback(
    async (
      ...
      const apiResponse: ApiKeyResponse =
        await createOrUpdateKeyViaEdgeFunction(userId, hashToUse, token);
  ...
  const refreshKey = useCallback(async (): Promise<{
    key: string;
    hash: string;
  }> => {
