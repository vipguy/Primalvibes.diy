...
class LazyDB {
  ...
  allDocs = async <T extends DocTypes>(
    options?: AllDocsQueryOpts,
  ): Promise<AllDocsResponse<T>> => this.inner.allDocs<T>(options);
...
export function useLazyFireproof(
  ...
  function useEnhancedDocument<T extends DocTypes>(
    doc: T,
  ): UseDocumentResult<T> {
    // Use a state counter to trigger refreshes when the database transitions
    const [refreshCounter, setRefreshCounter] = useState(0);
  ...
  function useEnhancedAllDocs<T extends DocTypes>(
    options?: AllDocsQueryOpts,
  ): AllDocsResult<T> {
    // Use a state counter to trigger refreshes when the database transitions
    const [refreshCounter, setRefreshCounter] = useState(0);
  ...
  const onDatabaseTransition = useCallback(
    (callback: (db: Database) => void) => {
      if (!ref.current) {
        console.warn(
          "Cannot subscribe to database transitions: database not initialized",
        );
        return () => {
          /* no-op */
        };
